...
engine = create_engine('sqlite:///fundemental.sqlite')
...
def query(*args):
    ...
    return Query(args).with_session(session).filter(fundamental.report_date < pd.to_datetime('2013-07-18')).group_by(
        fundamental.code)
...
class FundamentalReader(object):
    ...
    def query(self, dt, *args, **kwargs):
        ...
        return Query(args).with_session(self.session).filter(fundamental.report_date < dt)
