...
from QUANTAXIS.QAMarket.common import cn_en_compare, order_status_cn_en, trade_towards_cn_en
...
from QUANTAXIS.QAUtil.QAParameter import (BROKER_EVENT, BROKER_TYPE, ORDER_DIRECTION, ORDER_MODEL, ORDER_STATUS)
...
class QA_SPEBroker(QA_Broker):
    ...
    def call(self, func, params=''):
        try:
            if self.key == '':
                uri = '{}/api/v1.0/{}?client={}'.format(self._endpoint, func
                                                        , params.pop('client'))
            else:
                uri = '{}/api/v1.0/{}?key={}&client={}'.format(self._endpoint, func, self.key, params.pop('client'))
        ...
        try:
            ...
            if data is not None:
                ...
                return {'cash_available': cash_available,
                        'hold_available': hold_available.assign(amount=hold_available.amount.apply(float)).loc[:,
                                          ['code', 'amount']].set_index('code').amount}
            else:
                print(data)
    ...
    def query_orders(self, accounts, status='filled'):
        ...
        try:
            ...
            if data is not None:
                ...
                if status is 'filled':
                    return order_all.loc[:, self.dealstatus_headers].set_index(
                        ['account_cookie', 'realorder_id']).sort_index()
                else:
                    return order_all.loc[:, self.orderstatus_headers].set_index(
                        ['account_cookie', 'realorder_id']).sort_index()
            else:
    ...
    def send_order(self, accounts, code='000001', price=9, amount=100, order_direction=ORDER_DIRECTION.BUY,
