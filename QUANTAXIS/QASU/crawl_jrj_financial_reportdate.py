...
def QA_SU_save_report_calendar_day(client=DATABASE, ui_log=None, ui_progress=None):
    ...
    report_calendar = client.report_calenda
    ...
    for item in date_list:
        ...
        strProgressToLog = 'DOWNLOAD PROGRESS {}'.format(
            str(float((date_list.index(item) + 1) / len(date_list) * 100))[0:4] + '%', ui_log)
        intProgressToLog = int(float((date_list.index(item) + 1) / len(date_list) * 100))
        QA_util_log_info(strProgressToLog, ui_log=ui_log, ui_progress=ui_progress,
