...
def QA_fetch_get_analysis(code):
    """
    ...
    null = 'none'
    data = eval(requests.get(BusinessAnalysis_url.format(market, code), headers=headers_em).text)
    ...
    for item in zygcfx:
        try:
            data_ = pd.concat([pd.DataFrame(item['hy']).assign(data=item['rq']).assign(classify='hy'),
                               pd.DataFrame(item['cp']).assign(data=item['rq']).assign(classify='cp'),
                               pd.DataFrame(item['qy']).assign(data=item['rq']).assign(classify='qy')])
            temp.append(data_)
        except:
            pass
    try:
        res_zyfcfx = pd.concat(temp).set_index(['date', 'classify'], drop=False)
    except:
        res_zyfcfx = None
