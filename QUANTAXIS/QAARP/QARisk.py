...
class QA_Risk():
    ...
    def __init__(self, account, benchmark_code='000300', benchmark_type=MARKET_TYPE.INDEX_CN, if_fq=True,
        ...
        if self.account.markey_type == MARKET_TYPE.STOCK_CN:
            self.market_data = QA_fetch_stock_day_adv(self.account.code, self.account.start_date, self.account.end_date)
        elif self.account.markey_type == MARKET_TYPE.FUTURE_CN:
            self.market_data = market_data
        self.if_fq = if_fq

        if self.market_value is not None:
            self._assets = (self.market_value.sum(axis=1) + self.account.daily_cash.set_index('date').cash).fillna(
                method='pad')
        else:
            self._assets = self.account.daily_cash.set_index('date').cash.fillna(method='pad')

        self.time_gap = QA_util_get_trade_gap(self.account.start_date, self.account.end_date)
        self.init_cash = self.account.init_cash
        self.init_asswts = self.account.init_assets
    ...
    def market_value(self):
        ...
        if self.account.daily_hold is not None:
            if self.if_fq:
                return self.market_data.to_qfq().pivot('close').fillna(method='ffill') * self.account.daily_hold.apply(
                    abs)
            else:
                return self.market_data.pivot('close').fillna(method='ffill') * self.account.daily_hold.apply(abs)
    ...
    def max_dropback(self):
        ...
        return round(float(max(
            [(self.assets.iloc[idx] - self.assets.iloc[idx::].min()) / self.assets.iloc[idx] for idx in
             range(len(self.assets))])), 2)
...
class QA_Performance():
    ...
    def pnl_fifo(self):
        ...
        pnl = pnl.assign(pnl_ratio=(pnl.sell_price / pnl.buy_price) -
                                   1).assign(buy_date=pd.to_datetime(pnl.buy_date)).assign(
