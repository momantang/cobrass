...
from QUANTAXIS.QAUtil import (DATABASE, QA_util_log_info, QA_util_random_with_topic)
...
class QA_Portfolio():
    ...
    def add_account(self, account):
        "protfolio add a account/strategy"
    ...
    def pull(self, account_cookie=None, collection=DATABASE.account):
        ...
        if account_cookie is None:
            ...
            try:
                ...
                QA_util_log_info('{} sync successfully'.format(account_cookie))
            except Exception as e:
                QA_util_log_info(
...
class QA_PortfolioView():
    ...
    def trade_range(self):
        return pd.concat(
            [pd.Series(item.trade_range) for item in self.accounts]).drop_duplicates().sort_values().tolist()
